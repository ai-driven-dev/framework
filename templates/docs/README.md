...
The recommended flow from idea to pull request:

```text
/brainstorm → /new_issue → /plan → /deport → /implement + /assert + /commit → /review_code + /review_functional → /create_request
```
...
`/interview` → `/plan` → `/implement` → `/review_code` → `/commit` → `/create_request`
...
| `/structure_brief`    | Structure the final product brief from discovery     |
...
| `/deport`             | Isolate a task in a git worktree and implement       |
