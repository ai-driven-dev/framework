...
2. **Rewrite.** Carry the original text over, appending `[n]` to each verified claim, numbered in reading order.
3. **Surface.** For each conflict, state both sides in full ("Source A reports X; source B reports Y"), choosing no winner.
4. **Mark.** Append `(unverified - no source found)` to each unverified claim. Never delete it, never assert it.
