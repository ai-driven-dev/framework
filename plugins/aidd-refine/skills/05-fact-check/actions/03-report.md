...
The rewritten answer following `@${CLAUDE_PLUGIN_ROOT}/skills/05-fact-check/assets/report-template.md`: original content preserved, a `[n]` marker on each verified claim, a `(unverified - no source found)` marker on each unverified claim, conflicts stated with both sides, and a `## Sources` footnote block.
...
1. Copy `@${CLAUDE_PLUGIN_ROOT}/skills/05-fact-check/assets/report-template.md` as the structure.
