...
- Any raw verdict word - `verdict`, `claim false`, `claim true`, or the values `verified` / `conflict` / `unverified` used as a status (the inline `(unverified - no source found)` marker is the one allowed exception).
