...
- **After every ticked checkbox, commit atomically** via `aidd-vcs:commit`. One checkbox = one commit. Commit message: `<milestone-id>/<step>: <short description>`. This is non-negotiable — the audit trail is the safety net.
...
- `aidd-dev:plan` (read-only — for context)
- `aidd-dev:assert`
- `aidd-dev:test`
- `aidd-dev:debug`
- `aidd-dev:audit`
