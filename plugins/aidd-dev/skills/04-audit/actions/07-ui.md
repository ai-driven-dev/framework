...
The `ui` findings, written to `aidd_docs/tasks/audits/<yyyy>_<mm>_ui.md`, or contributed to the merged `<yyyy>_<mm>_full.md` in a full run.
...
3. **Rate.** Give each finding a severity and an effort per the `@../assets/audit-template.md` legend, with a concrete `file:line`. The category is always `ui`.
