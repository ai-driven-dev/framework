...
3. **Mark.** Set the plan frontmatter `status: in-progress` as a runtime marker. Do not commit this marker separately; include it at the first authorized commit checkpoint, or with `implemented` if there is no phase to code.
