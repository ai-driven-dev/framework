...
4. **Blocked.** If the implementer surfaces `BLOCKED` in `notes`, or the retry budget is spent, write `status: blocked` on `plan_path`, stop (do NOT proceed to 04), and escalate to a human.
