...
`aidd-async-dev` is an orchestration plugin. It does not implement the development lifecycle itself. It listens for two GitHub triggers (a label or a comment mention) and delegates the actual work to whichever SDLC orchestration skill is loaded in the runtime, discovered by description match.
...
| Skill                             | Purpose                                                         |
| --------------------------------- | --------------------------------------------------------------- |
| `aidd-async-dev:01:setup`         | Generate the workflow, write the config, bootstrap the labels.  |
| `aidd-async-dev:02:run`           | Pick a candidate, resolve blockers, lock, delegate, audit.      |
| `aidd-async-dev:03:review`        | Loop on PR feedback, reply per comment, resolve threads, summarise. |
...
2. Run the `aidd-async-dev:01:setup` skill in your repo. It asks five questions:
...
| `.claude/aidd-async-dev.json`     | Runtime config (committed; no secrets).                         |
