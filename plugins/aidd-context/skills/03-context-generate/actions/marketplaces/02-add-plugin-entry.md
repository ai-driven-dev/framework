...
4. For relative-path sources, verify the target directory exists under `<repo-root>` and contains the tool's expected manifest file (e.g. `.claude-plugin/plugin.json` for Claude Code, `.codex-plugin/plugin.json` for Codex CLI). For `github` / `url` / `git-subdir` / `npm` sources, no local check.
