...
4. For relative-path sources, verify the target directory exists under `<project_root>` and contains the tool's expected manifest file (e.g. `.claude-plugin/plugin.json` for Claude Code, `.cursor-plugin/plugin.json` for Cursor, `.codex-plugin/plugin.json` for Codex CLI). For `github` / `url` / `git-subdir` / `npm` sources, no local check.
...
For each confirmed (non-blocked) tool, the new entry is present in the tool's marketplace file `plugins[]`; for a Claude Code marketplace, `claude plugin validate .` reports zero errors; for a relative-path source, the tool's expected manifest file resolves under `<project_root>/<source>/`.
