...
8. **Save.** Write the completed agent file to each confirmed supported tool's native agents location using its path, naming, and extension conventions from `@${CLAUDE_PLUGIN_ROOT}/skills/03-context-generate/references/ai-mapping.md`. Paths are CWD-relative; write them directly under the workspace root (e.g. `.claude/agents/<name>.md`). Never resolve these paths relative to the plugin install directory.
...
9. **Post-write path check (MANDATORY).** After writing, MUST verify that every file in `files_written` satisfies BOTH:
