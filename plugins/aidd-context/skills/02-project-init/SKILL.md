...
- **Memory files location.** Memory templates (`scope: all | frontend | backend`) generated by action 03 from `assets/templates/memory/*.md` MUST be written at the ROOT of `aidd_docs/memory/`, NOT under any subdirectory. Example: `aidd_docs/memory/architecture.md`, NOT `aidd_docs/memory/internal/architecture.md`.
- **`aidd_docs/memory/internal/` reserved scope.** This subdirectory is reserved for AIDD-internal workflow traces (project-init audit notes, learn-captured decisions, async-dev pipeline state). Files generated from `assets/templates/memory/` NEVER land in `internal/`.
