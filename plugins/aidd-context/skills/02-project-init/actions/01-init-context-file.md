...
3. **Resolve target paths**. From `confirmed_tools`, map each to its context file per the mapping reference. Deduplicate (multiple tools may share `AGENTS.md`). The three canonical paths are the only valid targets: `<project_root>/CLAUDE.md`, `<project_root>/AGENTS.md`, `<project_root>/.github/copilot-instructions.md`. Any other file (`*.agent.md`, `<vendor>-*.md`, lookalikes) is user content and MUST NOT be touched.
