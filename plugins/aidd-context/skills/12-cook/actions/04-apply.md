...
1. **Locate.** Resolve the recipe with `@../references/recipe-locations.md` and read it. Run `list` first when the recipe is unnamed or when the user gives a number but no current numbered list is available.
