...
2. **Plan.** Turn each `## Steps` item into one todo entry.
...
- Applying a recipe produces a todo list mirroring its `## Steps`, asks for confirmation before executing, and runs the `## Verify` checks at the end.
