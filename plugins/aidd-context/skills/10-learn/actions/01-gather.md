...
1. **Require.** Learn writes into an existing memory bank. If `aidd_docs/memory/` is absent, route the user to the project-memory skill and stop. Never scaffold here.
2. **Pick the source.** Offer three and let the user choose, or take the one the user named:
...
3. **Collect.** From the chosen source, pull the raw signals: a stated rule or convention, a decision and its reason, a deprecation, a pivot that cost time, a pattern worth reusing, a pitfall worth avoiding. Read git changes for what changed and why, not line by line.
4. **Drop noise.** Remove anything personal, an AI preference, a routine edit, part of AIDD's own scaffold, or already captured in memory, a rule, or a decision. These never reach scoring.
