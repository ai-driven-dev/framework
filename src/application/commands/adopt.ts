...
export function registerAdoptCommand(program: Command): void {
  program
    ...
    .requiredOption("-t, --tools <tools>", "Comma-separated list of installed tools (claude, cursor, copilot)")
    .option("-d, --docs-dir <dir>", "Documentation directory", "aidd_docs")
    .action(async (cmdOptions: { tools: string; docsDir: string }) => {
      const globalOptions = program.opts<{ verbose: boolean; repo?: string; token?: string; release?: string }>();
      ...
      try {
        if (!globalOptions.release) {
          throw new Error("--release <version> is required for adopt. Example: aidd adopt --release 3.3.3 --tools claude");
        }

        const toolIds = cmdOptions.tools.split(",").map((t) => t.trim()) as ToolId[];
        const deps = await createDeps(projectRoot, { verbose, repo: globalOptions.repo, token: globalOptions.token }, output);
