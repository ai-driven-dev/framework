...
describe.concurrent("E2E: aidd setup (adopt flow)", () => {
  it("detects pre-existing AIDD files and registers them without overwriting", async () => {
    ...
    try {
      ...
      const { stdout, exitCode } = await runCli(
        ["setup", "--ai", "claude", "--from", FRAMEWORK_PATH, "--path", FRAMEWORK_PATH],
        projectDir
      );
  ...
  it("fails in non-interactive mode without tool flags when adopt state is detected", async () => {
    ...
    try {
      ...
      // No tool flags in non-interactive mode — should fail
      const { stderr, exitCode } = await runCli(["setup", "--path", FRAMEWORK_PATH], projectDir);

      expect(exitCode).toBe(1);
      expect(stderr.toLowerCase()).toMatch(/non-interactive/);
    } finally {
      await cleanup();
    }
  });
  ...
  it("fails in non-interactive mode without --from flag when adopt state is detected", async () => {
    ...
    try {
      ...
      // --ai is provided but --from is missing — should fail with error about --from
  ...
  it("adopts multiple tools from a local framework path", async () => {
    ...
    try {
      ...
      const { stdout, exitCode } = await runCli(
        ["setup", "--ai", "claude,copilot", "--from", FRAMEWORK_PATH, "--path", FRAMEWORK_PATH],
        projectDir
      );
