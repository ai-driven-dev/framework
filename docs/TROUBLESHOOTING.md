...
- **Authored for Claude Code.** Other tools install via their native mechanism from the release archives (see [Other tools](../README.md#other-tools)); publishing to each tool's public marketplace is on the way, and native parity is a roadmap item, not a guarantee today.
