...
- `status` detects 3 drift types: `modified` (hash mismatch), `deleted` (missing from disk), `added` (on disk but not tracked). Files ending in `.backup` are excluded from the untracked-file scan. Also performs a best-effort version check via `FrameworkResolver.fetchLatestVersion()` — network failure is swallowed silently. Non-semver versions excluded from comparison. `compareSemver(a, b)` defined in `domain/models/semver.ts`, re-exported from `status-use-case.ts`. Update info shown first (docs → `aidd init --force`, tools → `aidd install --all`). `--tool <toolId>` filters to a single tool (mutually exclusive with `--docs`). `--docs` filters to docs only (tools skipped). Docs drift includes: tracked file changes, CATALOG.md existence check (not manifest-tracked but reported as `deleted` if missing, skipped in untracked scan), and untracked files in docs dir (reported as `added`, CATALOG.md excluded).
- `check-update.ts` (`application/check-update.ts`): `printUpdateBanner(resolver, manifestRepo, logger)` — best-effort update check printed before every command via a `program.hook("preAction", ...)` in `cli.ts`. Reads version from manifest (docs first, then any tool), compares with latest semver, prints context-aware guidance: both outdated → `aidd update`; docs only → `aidd update --docs`; tools only → `aidd update`. Silent on null manifest, non-semver versions, or resolver errors. Not called individually per command.
...
- `CATALOG.md` is generated (not installed): after every `init`, `install`, and `uninstall`, `writeCatalog()` writes `{docsDir}/CATALOG.md` with markdown tables linking to each installed file. The framework's own `CATALOG.md` is skipped during `init` docs installation. CATALOG is not tracked in the manifest but is checked by `status` — reported as `deleted` if missing. `clean --force` deletes it explicitly alongside manifest-tracked docs files.
- `update` diffs the new framework distribution against both the manifest and disk: `added` = new file in framework, `removed` = deleted from framework, `changed` = framework content differs OR disk file is missing/modified (conflict if user also modified disk). `--dry-run` computes diff without writing. `--force` overwrites conflicts without prompting. Merged files (e.g., `.vscode/settings.json`) are always re-applied silently regardless of conflict status. `--tool <toolId>` limits update to a single tool (docs skipped). `--docs` limits update to docs only (tools skipped). `--tool` and `--docs` are mutually exclusive.
- `restore` detects `modified` (disk hash ≠ manifest hash) and `deleted` files. Does NOT touch untracked/user-added files — use `clean --force` for a full reset. Uses pinned version from manifest; falls back to latest with warning if pinned version unavailable. `--tool` scopes to a single tool (docs skipped). `--docs` scopes to docs only (tools skipped). `--tool` and `--docs` are mutually exclusive. Manifest is updated in one `addTool()` call after all restores complete (not per-file — avoids manifest corruption on multi-file restore). Without `--force`, prompts interactively via `Prompter.resolveConflict(path, reason)` where `reason` is `"deleted"` or `"modified"` — message differs accordingly. Aborts with exit 1 if non-TTY and `--force` is not set ("Restore requires --force in non-interactive mode").
...
- Skills can contain plain-text prose references to command paths (e.g. `{{TOOLS}}/commands/02_context/brainstorm.md`). `rewriteContent` handles both `@`-prefixed (@-include) and non-`@` (prose) variants: Claude normalizes both to `.claude/commands/aidd/{phase}/file.md` via a single regex `(@?)\.claude\/commands\/(\d+)[_][^/]+\/`; Cursor leaves `02_context/` intact (correct install path); Copilot applies `flattenFileName` to produce `.github/prompts/02-file.prompt.md`.
