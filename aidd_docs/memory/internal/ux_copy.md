...
| `error.network.unreachable`        | Cannot reach the framework source. Check your network connection.                                            | Verify network or use `--path` with local source          | Network failure, no cache available                          |
| `error.tarball.invalid`            | Downloaded file is not a valid tarball                                                                       | Re-download or provide a valid `.tar.gz` via `--path`     | Corrupted download or wrong file format                      |
...
| `error.descriptor.missing_dir`     | No framework descriptor found in the specified directory                                                     | Verify the directory contains `framework.json`                 | `--path` points to a directory without `framework.json` |
