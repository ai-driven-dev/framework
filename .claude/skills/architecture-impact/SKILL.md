...
description: 'Analyzes the precise impact of a brownfield change on existing architecture. Use when you need to map impacted modules and plan migrations for an evolution.'
---
...
4. Generate an impact tree diagram (Mermaid) with severity coloring
5. For each impacted module, document:
...
7. Choose evolution strategy using decision tree:
...
8. Plan data migrations at strategy level (if any):
...
9. Verify the impact analysis against these criteria:
...
10. Save as `aidd_docs/tasks/YYYY-MM-DD-{change-name}/architecture-impact.md`
...
| Input | `aidd_docs/memory/internal/architecture.md`     | Initial architecture decisions (if available) |
