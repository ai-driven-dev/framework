---
name: 'ux-flow-update'
description: 'Maps only the user flows impacted by a brownfield change, covering all states. Use when you need to document flow changes without remapping the entire product.'
---
...
6. For each impacted flow, document:
...
7. For each step in each impacted flow, document all states:
...
8. Document recovery paths for each error state
...
10. Verify format: state tables use response types (not exact copy), Mermaid flowcharts present
...
11. Save as `aidd_docs/tasks/YYYY-MM-DD-{change-name}/user-flows-update.md`
...
| Input    | `aidd_docs/memory/internal/system_overview.md` | Current system state                   |
| Input    | `aidd_docs/memory/internal/user_flows.md`      | Existing user flows (if available)     |
| Template | `aidd_docs/templates/ux/user_flows.md`         | User flows template (section structure)|
