...
description: 'Generates a progressive rollout plan with feature flags, monitoring, and rollback. Use when you need a concrete deployment plan for a brownfield change.'
---
...
4. For each feature flag, document:
...
5. Define progressive rollout stages:
   - Canary (1-5%) → 24-48h
   - Early Adopters (10-25%) → 3-5 days
   - Majority (50-75%) → 3-5 days
   - Full (100%)
6. Set progression criteria for each stage (error rate, latency, business metrics)
7. Document rollback procedure: step-by-step actions, estimated time, data loss assessment
8. Assess rollback reversibility for each component:
...
9. Verify the impact plan against these criteria:
...
10. Define monitoring dashboard: before/after baseline, flag status, error tracking, alert thresholds
11. Save as `aidd_docs/tasks/YYYY-MM-DD-{change-name}/impact-plan.md`
...
| ----- | ---------------------------------------------------------------- | --------------------------------------- |
| Input | `aidd_docs/tasks/YYYY-MM-DD-{change-name}/architecture-impact.md` | Impact analysis                         |
| Input | `aidd_docs/memory/internal/system_overview.md`                    | System overview                         |
| Input | `aidd_docs/memory/internal/milestones.md`                         | Initial milestone sequencing (if available) |
