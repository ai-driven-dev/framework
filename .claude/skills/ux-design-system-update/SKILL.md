...
description: 'Audits existing UI patterns and generates a design system update plan for brownfield evolution. Use when you need to integrate new components without breaking visual consistency.'
---
...
3. Identify which new components are needed vs which existing ones can be adapted
4. For each new component, specify: tokens, variants, states, accessibility, migration plan
...
5. Update guidelines: palette, typography, spacing, interaction patterns
6. Verify navigation coherence: breadcrumbs, URLs, responsive, transitions
...
7. Verify the design system update against these criteria:
...
8. Save as `aidd_docs/tasks/YYYY-MM-DD-{change-name}/design-system-update.md`
...
| ----- | ---------------------------------------- | --------------------- |
| Input | `aidd_docs/memory/internal/change_brief.md`     | Change brief (brownfield) or `prd.md` (greenfield) |
| Input | `aidd_docs/memory/internal/design_system.md`    | Existing design system (if available) |
