...
description: 'Generates accessibility specifications for new and modified components in a brownfield change. Use when you need a11y specs scoped to a change, not the full product.'
---
...
6. For each impacted component, specify:
...
7. Define focus management for new dynamic elements:
   - Focus trap for new modals/drawers
   - Focus restoration on close
   - Focus movement for new dynamic content
8. Verify contrast ratios for new design tokens:
   - Text on background: minimum 4.5:1 (AA)
   - Large text: minimum 3:1 (AA)
   - UI components: minimum 3:1 (AA)
9. Reference token names from design-system-update (not raw hex values)
...
11. Verify format: ARIA roles explicit, contrast references tokens not hex, keyboard sequences complete
...
12. Save as `aidd_docs/tasks/YYYY-MM-DD-{change-name}/accessibility-update.md`
...
| Input    | `aidd_docs/memory/internal/accessibility_spec.md`  | Existing a11y spec (if available, for patterns) |
| Template | `aidd_docs/templates/ux/accessibility_spec.md`     | Accessibility template (section structure)      |
