...
export class UserPluginUpdateUseCase {
  ...
  private async updateLocked(options: PluginUpdateOptions): Promise<string[]> {
    ...
    const nativeUpdates = await Promise.all(
      nativeTargets.map(async ({ toolId, claim }) => ({
        toolId,
        claim,
        activator: await this.nativeHost.requireTargetedUpdate(toolId, claim, options.projectRoot),
      }))
    );
    ...
    for (const { toolId, plugin } of targets) {
      ...
      const next = await this.fileUpdater.update(plugin, toolId, options.projectRoot, this.logger);
