...
4. If no candidate matches, post a comment on the issue explaining that an SDLC orchestration skill must be loaded, release the lock by removing `ai:running` and re-adding `ai:ready`, and abort the cycle with a non-zero exit so the run is marked failed.

## Test

In a sandbox where no SDLC-advertising skill is loaded, running this action posts the missing-capability comment, swaps labels back, and exits non-zero. With at least one such skill loaded, it returns `sdlc_available = true` and the chosen skill name without side effects.
